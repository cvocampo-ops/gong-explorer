...
export function Header() {
  const { isConfigured, clearCredentials, credentials, provider } = useCredentials();

  const providerLabel = provider === "salesloft" ? "SalesLoft" : "Gong";
  const hostHint =
    credentials?.provider === "gong"
      ? credentials.baseUrl.replace("https://", "")
      : credentials?.provider === "salesloft"
      ? "api.salesloft.com"
      : "";
    ...
    <header className="sticky top-0 z-50 border-b border-white/[0.06] glass-strong">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-3">
          {isConfigured ? (
            <>
              <Badge variant="outline" className="gap-1.5 rounded-full border-purple-500/30 bg-purple-500/10 text-xs text-purple-400">
                {providerLabel}
              </Badge>
