...
export function BulkDownloadModal({ state, onCancel, onClose }: BulkDownloadModalProps) {
  ...
  const isActive = state.status === "starting";
      ...
      <div className="glass-strong mx-4 w-full max-w-md rounded-2xl border border-white/10 p-6 shadow-2xl">
        ...
        <div className="mb-6 flex items-center justify-between">
          <h3 className="text-lg font-semibold">
            {isActive && "preparing download..."}
            {state.status === "complete" && "download started"}
        ...
        {isDone && (
          <div className="mb-4 space-y-2">
            {state.status === "complete" && state.failures.length === 0 && (
              <div className="flex items-center gap-2 text-sm text-green-400">
                <CheckCircle className="h-4 w-4 shrink-0" />
                <span>
                  zip export is streaming to your browser. check your downloads.
                </span>
              </div>
            )}
            {successCount > 0 && state.status !== "complete" && (
              <div className="flex items-center gap-2 text-sm text-green-400">
                <CheckCircle className="h-4 w-4 shrink-0" />
                <span>
